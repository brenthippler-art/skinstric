...
export default function DemographicPanel({
  ...
  return (
    <div className="relative w-full bg-[#F3F3F4] px-4 py-6 md:px-0 md:py-0 md:[height:min(56.67vh,544px)] md:[width:min(60.83vw,1168px)]">
      ...
      <div ref={contentRef} className="relative md:h-full">
...
function CircleChart({ percentage }: { percentage: number }) {
  ...
  useLayoutEffect(() => {
    gsap.to(progress.current, {
      ...
      ease: "power2.out",
      onUpdate: () => {
        const dashOffset = CIRCUMFERENCE * (1 - progress.current.value / 100);
        arcRef.current?.setAttribute("stroke-dashoffset", String(dashOffset));
      },
    });
  }, [percentage]);
    ...
    <div className="relative h-full w-full">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="flex items-baseline text-[#1A1B1C]">
          ...
          <span className="text-[clamp(0.75rem,1.3vw,1.5rem)] font-normal tracking-[-0.05em]">
            %
          </span>
        </span>
      </div>
